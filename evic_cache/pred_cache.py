...
def get_pred(rank, world_size, data, max_length, max_gen, prompt_format, dataset, device, model_name, model2path, out_path):
    ...
    kv_cache = enable_streaming_llm(
        model, start_size=1, recent_size=255
    )
    ...
    for json_obj in tqdm(data):
        ...
        if kv_cache is not None:
            space_needed = 0.5*context_length + max_gen
            # space_needed = context_length + 10000
            past_key_values = kv_cache.evict_for_space(past_key_values, space_needed)

        if dataset == "samsum": # prevent illegal output on samsum (model endlessly repeat "\nDialogue"), might be a prompting issue
            pred = greedy_generate(model, tokenizer, input.input_ids, max_gen, past_key_values=past_key_values)
...
def greedy_generate(model, tokenizer, input_ids, max_gen_len, past_key_values=None):
    outputs = model(
        input_ids=input_ids,
...
if __name__ == '__main__':
    ...
    datasets = ["2wikimqa", "multifieldqa_zh", "hotpotqa", "passage_count","passage_retrieval_en","multifieldqa_en", "narrativeqa","musique", "triviaqa"]
